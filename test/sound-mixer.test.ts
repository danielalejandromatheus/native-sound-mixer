import SoundMixer, { DeviceType } from '../dist/sound-mixer.js'
describe('sound mixer', () => {
  it('should get all devices', () => {
    const devices = SoundMixer.devices
    expect(
      devices.filter(({ type }) => type === DeviceType.RENDER).length
    ).toBeGreaterThanOrEqual(1)
    expect(
      devices.filter(({ type }) => type === DeviceType.CAPTURE).length
    ).toBeGreaterThanOrEqual(1)
  })
