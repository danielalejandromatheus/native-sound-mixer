...
const sMixerModule: { SoundMixer: SoundMixer } = (() => {
  const getModule = (platform: platform, arch: arch = undefined) => {
    ...
    const path = `${__dirname}/addons/${platform}-sound-mixer${archString}.node`
...
export declare class AudioSession {
  ...
  /**
   * The process id of the application using the {@link AudioSession}.
   * @readonly
   */
  ...
  /**
   * @param {string} event - The type of event that will be removed from the event pool
   * @param {number} handler - The handler ID of the registered event to be removed
   * @returns whether the event was unregistered or not
   * @see {@link AudioSession.on | registering a listener}
   */
  public removeListener(event: string, handler: number): ()=> boolean
}
